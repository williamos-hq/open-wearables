...
"og:url": "https://docs.openwearables.io/providers/garmin-bridge-import"
"og:type": "article"
---
...
Open Wearables accepts only the kind and source-method pairs in its small
Python normalization allowlist and rejects arbitrary method names before any
write. The bridge owns polling cadence, stable-key construction, and no-data
behavior; those transport details are intentionally not duplicated here.
...
|---|---|---|
| `200` | Import transaction committed; inspect counts and normalization errors | Advance acknowledged cursor |
| `204` | Provider deletion committed | Treat the purge as complete |
...
Deleting an already absent user or already purged Garmin dataset also returns
`204`, so bridge retries are safe.
