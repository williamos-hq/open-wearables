...
Apple Health integration uses a **push-based model** where health data is sent from the user's device to Open Wearables using our mobile SDK. This is different from official cloud providers like Polar that use OAuth and webhooks. Garmin uses the private bridge import in this fork.
