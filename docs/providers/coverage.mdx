...
<Accordion title="Garmin">
  **Integration Method:** Private, authenticated bridge import

  - A separate bridge owns Garmin Connect authentication, MFA, polling, overlap windows, cooldowns, and cursors
  - Open Wearables accepts only checked-in health and activity methods through a bounded, versioned contract
  - Scrubbed native observations preserve unknown fields while stable concepts are normalized
  - Full sleep processing with stages (deep, light, REM, awake), duration, and efficiency score
  ...
  - 135+ workout type mappings to the shared taxonomy, plus distance, calories, heart rate, elevation gain, speed, and cadence
  - Original FIT files preserve routes, samples, laps, zones, device provenance, and strength-set metadata without expanding every sample into PostgreSQL
  - Official Garmin OAuth, webhooks, vendor reads, and backfill routes are unavailable in this fork
</Accordion>
