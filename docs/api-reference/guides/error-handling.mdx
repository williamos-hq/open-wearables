...
```json
{
  "detail": "User not connected to polar"
}
```
...
```bash
GET /api/v1/oauth/polar/authorize?user_id={user_id}
```
...
```json
{
  "detail": "Token expired and no refresh token available for polar"
}
```
...
```json
{
  "detail": "Polar authorization expired. Please re-authorize."
}
```
