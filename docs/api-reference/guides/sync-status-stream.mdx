...
description: "Stream real-time sync lifecycle events for pulls, webhooks, SDK uploads, and file imports via Server-Sent Events."
...
The sync status API lets you observe the lifecycle of Open Wearables pulls, webhooks, SDK uploads, and Apple Health XML imports in real time. Garmin bridge history is surfaced by WilliamOS rather than this API.
...
{
  "event_id": "evt_01HZ...",
  "run_id": "pull_oura_user42_1730000000",
  "user_id": "user42",
  "provider": "oura",
  "source": "pull",
  "stage": "fetching",
...
event: sync.status
data: {"event_id":"evt_01HZ...","run_id":"pull_oura_user42_1730000000","provider":"oura","stage":"fetching","status":"in_progress","progress":0.42,...}

: heartbeat

event: sync.status
data: {"event_id":"evt_01HZ...","run_id":"pull_oura_user42_1730000000","provider":"oura","stage":"completed","status":"success","progress":1.0,"ended_at":"2024-10-27T10:16:45Z",...}
```
...
Returns the **latest** event for each unique `run_id` from the past 24 hours (one row per run), ordered newest first. Each `run_id` is stable for the entire lifetime of a single sync invocation, and only its most recent event is surfaced here.
