...
Garmin is **import-only** in this fork. The private `garmin-bridge` owns account
login, MFA, polling, cursors, and request budgets. Open Wearables accepts only
the dedicated service-to-service ingest contract and serves the resulting data
through its normal summaries, timeseries, workout, sleep, and score endpoints.

Official Garmin OAuth, webhook, sync, and backfill endpoints return `404`. See
the [Garmin Bridge guide](/providers/garmin-data-pipeline) for configuration,
contract examples, FIT retention, and deletion behavior.
