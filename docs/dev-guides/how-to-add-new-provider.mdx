...
| `webhook_callback` | Provider-initiates async export delivered to our webhook | Export-style integrations |
| `webhook_stream` | Provider pushes full data payload in every webhook | Suunto |
...
<Note>
`webhook_stream` and `webhook_ping` are mutually exclusive. Use `webhook_stream` when the provider embeds the complete record in the webhook body (Suunto). Use `webhook_ping` when the webhook is only a trigger and you must call the REST API to retrieve the actual data (Oura, Strava, Fitbit, Polar). Garmin is import-only in this fork and uses neither public webhook mode.
</Note>
...
```python
# Full-payload stream (Suunto-style) — historical pull + live webhook stream
