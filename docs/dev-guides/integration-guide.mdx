...
<Note>
Garmin is import-only in the WilliamOS fork and does not participate in the OAuth
flow below. Its private bridge populates the same normalized read endpoints. See
the [Garmin Bridge guide](/providers/garmin-data-pipeline).
</Note>
...
Returned when `live_sync_mode` is included for a provider where `live_sync_configurable` is `false` (for example import-only Garmin).
...
<Warning>
OAuth provider names must be **lowercase**. Garmin OAuth is unavailable in this fork.
</Warning>

<CodeGroup>

```bash cURL
curl "http://localhost:8000/api/v1/oauth/polar/authorize?user_id=176be8de-8452-4eb7-a7ea-147fec925d9d" \
  -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
```
...
<Steps>
  <Step title="User clicks 'Connect Polar'">
...
```bash
curl "http://localhost:8000/api/v1/oauth/polar/authorize?user_id=USER_ID&redirect_uri=https://yourapp.com/oauth/callback" \
  -H "X-Open-Wearables-API-Key: YOUR_API_KEY"
```
...
Garmin history is scheduled and imported by the private bridge, not this OAuth backfill path.
...
async def main():
    ...
    polar_connected = any(
        c["provider"] == "polar" and c["status"] == "active"
...
<AccordionGroup>
  ...
  <Accordion title="Data endpoints return empty results right after OAuth">
    OAuth-provider backfill is dispatched asynchronously on connect and can take anywhere from seconds to several minutes depending on the provider and the amount of history. Poll `GET /api/v1/users/{user_id}/connections` - `last_synced_at` flips from `null` to a timestamp once the first sync finishes. Garmin progress is exposed by WilliamOS through the private bridge.
  ...
  <Accordion title="OAuth callback not redirecting to my app">
    ...
    ```bash
    /api/v1/oauth/polar/authorize?user_id=...&redirect_uri=https://myapp.com/callback
    ```
  </Accordion>
</AccordionGroup>
